...
### [Planner/Executor] 전체 UI·UX 개선 — 2026-09-24

사용자: 전체 UI 및 UX 개선.

원칙: 페이지마다 다른 폭·제목·가짜 스크린샷을 없애고, 경력 대표작과 같은 언어로 맞춘다.

- 공통 컨테이너 max-w-6xl, PageHeader, 건너뛰기 링크, 헤더 활성 표시·연락 CTA, 푸터 바로가기
- 홈 대표작 = CompanyFlow / Boardroom / 환자 분석. 히어로 와이어프레임 제거
- 소개 타임라인·원칙, 스킬 리스트, 연락 복사·teal 포커스, 프로젝트 대표작+칩
- 버튼을 링크로 직접 써서 중첩 클릭 제거
...
## [Executor] 홈 벤토 레이아웃 정렬 — 2026-09-24

사용자 스크린샷: 소개 이미지가 가로로 잘리고, 인트로(2칸)·CompanyFlow(4칸)·Boardroom(4칸)·환자(2칸) 높이가 안 맞음.

원인: 짧은 고정 높이(`h-36`)를 넓은 4칸에 넣어 16:10 스크린샷이 16:3처럼 잘림.

수정: 인트로는 한 줄, 대표작 3칸 동일 폭, 미디어 영역을 스크린샷과 같은 `aspect-[16/10]`로 통일.
