import { Career } from '@/types/career';
import Badge from '@/components/ui/Badge';
import { formatDate } from '@/lib/utils/formatDate';
import { calculateDuration } from '@/lib/utils/calculateDuration';
import { MapPin, Calendar, Github, ExternalLink, Building2 } from 'lucide-react';

interface CareerCardProps {
  career: Career;
}

const statusLabels = {
  active: '재직중',
  completed: '완료',
  'in-progress': '진행중',
} as const;

/** 시설 경력(또는 레거시)용 — 전체 카드 onClick 없음, 링크만 명시 */
export default function CareerCard({ career }: CareerCardProps) {
  const duration = career.duration || calculateDuration(career.period.start, career.period.end);

  return (
    <article className="border-l-2 border-l-accent bg-white py-1 pl-5">
      <div className="flex flex-col gap-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            <span className="mt-0.5 inline-flex h-8 w-8 shrink-0 items-center justify-center bg-teal-50 text-accent">
              <Building2 size={16} aria-hidden />
            </span>
            <div>
              <h3 className="text-lg font-semibold text-ink">{career.title}</h3>
              <p className="mt-1 text-sm text-ink-secondary">{career.company}</p>
              <p className="text-sm text-ink-muted">{career.position}</p>
            </div>
          </div>
          {career.status && <Badge variant={career.status}>{statusLabels[career.status]}</Badge>}
        </div>

        <div className="flex flex-wrap items-center gap-4 text-xs text-ink-muted">
          <div className="flex items-center gap-1">
            <Calendar size={14} aria-hidden />
            <span>
              {formatDate(career.period.start)} ~ {formatDate(career.period.end)}
            </span>
            {duration && <span className="ml-1">({duration})</span>}
          </div>
          {career.location && (
            <div className="flex items-center gap-1">
              <MapPin size={14} aria-hidden />
              <span>{career.location}</span>
            </div>
          )}
        </div>

        {career.description && career.description.length > 0 && (
          <ul className="space-y-2">
            {career.description.map((desc) => (
              <li key={desc} className="flex items-start gap-2 text-sm text-ink-secondary">
                <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-ink-muted" aria-hidden />
                <span>{desc}</span>
              </li>
            ))}
          </ul>
        )}

        {career.responsibilities && career.responsibilities.length > 0 && (
          <div>
            <h4 className="mb-2 text-xs font-medium uppercase tracking-wide text-ink-muted">주요 업무</h4>
            <ul className="space-y-1">
              {career.responsibilities.map((resp) => (
                <li key={resp} className="flex items-start gap-2 text-sm text-ink-secondary">
                  <span className="mt-0.5 text-ink-muted">-</span>
                  <span>{resp}</span>
                </li>
              ))}
            </ul>
          </div>
        )}

        {career.achievements && career.achievements.length > 0 && (
          <div>
            <h4 className="mb-2 text-xs font-medium uppercase tracking-wide text-ink-muted">성과</h4>
            <ul className="space-y-1">
              {career.achievements.map((achievement) => (
                <li key={achievement} className="flex items-start gap-2 text-sm text-ink-secondary">
                  <span className="mt-0.5 text-accent" aria-hidden>
                    ✓
                  </span>
                  <span>{achievement}</span>
                </li>
              ))}
            </ul>
          </div>
        )}

        {career.techStack && career.techStack.length > 0 && (
          <div className="mt-1 flex flex-wrap gap-2">
            {career.techStack.map((tech) => (
              <span
                key={tech}
                className="border border-line bg-surface px-2 py-0.5 text-xs text-ink-secondary"
              >
                {tech}
              </span>
            ))}
          </div>
        )}

        <div className="flex flex-wrap gap-4">
          {career.demo && (
            <a
              href={career.demo}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-sm font-medium text-ink-body underline decoration-line underline-offset-2 hover:decoration-ink-muted"
            >
              <ExternalLink size={16} aria-hidden />
              사이트 보기
            </a>
          )}
          {career.github && (
            <a
              href={career.github}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-sm text-ink-secondary hover:text-ink"
            >
              <Github size={16} aria-hidden />
              GitHub 리포지토리
            </a>
          )}
        </div>
      </div>
    </article>
  );
}
